...
# Assuming img is your input array
img = np.random.randint(0, 255, size=(270000))

# Reshape the array into a 3D array with dimensions (300, 300, 3)
# Here, we assume that the input image has a shape compatible with (300, 300, 3)
# Adjust the shape accordingly based on the actual dimensions of your image
reshaped_img = img.reshape(300, 300, 3)

# Perform further operations with reshaped_img



# Loading Models
covid_model = load_model('models/covid19_detection_model.h5')
...
pneumonia_model = load_model('models/pneumonia_detection_model.h5')
...
def resultp():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            print(pneumonia_model.summary())

            # Resize input images to match expected input shape
            img = cv2.imread('static/uploads/' + filename)
            if img is not None:
                # Resize image to match expected input shape of the model
                img = cv2.resize(img, (300, 300))  # Adjust dimensions as needed
                img = np.expand_dims(img, axis=0)  # Add a batch dimension
